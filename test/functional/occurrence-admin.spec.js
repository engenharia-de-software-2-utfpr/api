...
test('altera status de uma ocorrência', async ({ assert, client }) => {
  ...
  await client.put('occurrence-admin/' + occurrenceObj.id + '/status').send({ status: 'approved' }).end()
...
test('lista todas as ocorrências', async ({ assert, client }) => {
  ...
  const response = await client.get('occurrence-admin').end()
...
test('lista todas as ocorrências pendentes', async ({ assert, client }) => {
  ...
  const response = await client.get('occurrence-admin').query({ status: 'waiting' }).end()
...
test('lista todas as ocorrências aprovadas', async ({ assert, client }) => {
  ...
  const response = await client.get('occurrence-admin').query({ status: 'approved' }).end()
